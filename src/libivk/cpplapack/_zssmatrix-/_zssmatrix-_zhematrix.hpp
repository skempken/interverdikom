...
inline _zgematrix operator+(const _zssmatrix& matA, const _zhematrix& matB)
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] operator+(const _zssmatrix&, const _zhematrix&)"
            << std::endl;
#endif//CPPL_VERBOSE
  
#ifdef  CPPL_DEBUG
  if(matA.M!=matB.N || matA.N!=matB.N){
    std::cerr << "[ERROR] operator+(const _zssmatrix&, const _zhematrix&)"
              << std::endl
...
inline _zgematrix operator-(const _zssmatrix& matA, const _zhematrix& matB)
{
  ...
  //// shallow copy to zgematrix and change sign ////
  zgematrix newmat(-matB);
