...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] dgematrix::operator=(const dgbmatrix&)"
            << std::endl;
#endif//CPPL_VERBOSE
...
#ifdef  CPPL_DEBUG
  if(N!=mat.N || M!=mat.M){
    std::cerr << "[ERROR] dgematrix::operator+=(const dgbmatrix&)" << std::endl
...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] operator+(const dgematrix&, const dgbmatrix&)"
            << std::endl;
#endif//CPPL_VERBOSE
