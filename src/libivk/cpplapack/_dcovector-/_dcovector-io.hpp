...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] _dcovector::operator()(const long&) const"
            << std::endl;
#endif//CPPL_VERBOSE
  
#ifdef  CPPL_DEBUG
  if( i<0 || L<=i ){
    std::cerr << "[ERROR] _dcovector::operator()(const long&) const"
              << std::endl
...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] operator<<(std::ostream&, const _dcovector&)"
            << std::endl;
#endif//CPPL_VERBOSE
...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] _dcovector::write(const char*) const"
            << std::endl;
#endif//CPPL_VERBOSE
