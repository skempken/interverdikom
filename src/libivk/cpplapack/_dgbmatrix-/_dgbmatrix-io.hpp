...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] _dgbmatrix::operator()(const long&, const long&) const"
            << std::endl;
#endif//CPPL_VERBOSE
  
#ifdef  CPPL_DEBUG
  if( i<0 || j<0 || M<=i || N<=j || i-j>KL || j-i>KU ){
    std::cerr << "[ERROR] _dgbmatrix::operator()(long, long)" << std::endl
...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] operator<<(std::ostream&, const _dgbmatrix&)"
            << std::endl;
#endif//CPPL_VERBOSE
...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] _dgbmatrix::write(const char*) const"
            << std::endl;
#endif//CPPL_VERBOSE
