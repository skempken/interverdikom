...
inline std::complex<double>& _zrovector::operator()(const long& i) const
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] _zrovector::operator()(const long&) const"
            << std::endl;
#endif//CPPL_VERBOSE
  
#ifdef  CPPL_DEBUG
  if( i<0 || L<=i ){
    std::cerr << "[ERROR] _zrovector::operator()(const long&) const"
              << std::endl
...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] operator<<(std::ostream&, const _zrovector&)"
            << std::endl;
#endif//CPPL_VERBOSE
...
{
#ifdef  CPPL_VERBOSE
  std::cerr << "# [MARK] _zrovector::write(const char*) const"
            << std::endl;
#endif//CPPL_VERBOSE
