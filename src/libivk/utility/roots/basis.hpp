...
#ifdef __EMX__            /* emx 0.9b mit GNU CC 2.7.2?               */
#if __GNUC__ == 2         /* Um einem Compilerabsturz in `15\gax.c'   */
#if __GNUC_MINOR__ == 7   /* vorzubeugen, wenn man mit LDOUBLE        */
#define EMX09B            /* uebersetzt, werden hier durch das Makro  */
#define _WITH_UNDERSCORE  /* _WITH_UNDERSCORE die speziellen          */
#define fabsl _fabsl      /* Varianten der mathematischen Funktionen  */
#define sqrtl _sqrtl      /* fuer `long double' in `math.h' aktiviert */
#define powl  _powl       /* und ihre Benutzung weiter unten in Form  */
#define sinl  _sinl       /* der Makros FABS, SQRT, usw. vorbereitet, */
#define cosl  _cosl       /* was ja auch ohne die Angelegenheit mit   */
#define expl  _expl       /* dem abstuerzenden Compiler ganz          */
#define logl  _logl       /* nuetzlich ist.                           */
#define atanl _atanl
#define acosl _acosl
#define coshl _coshl
#endif
...
***********************************************************************/
//-> GEAENDERT: wegen "QGLOBAL_H" von Qt
/*
#undef boolean
#undef FALSE
#undef TRUE
...
//namespace basis_h{
#define ENGELN_ZERO      (REAL)0.0       /* Namen fuer haeufig vorkommende   */
/*.IX{ZERO}*/
//}
...
#if defined(LDOUBLE) && (defined(BC3) || defined(MC6) || defined(EMX09B))
...
//#undef min
//#undef max
#define signy(x, y) (((y) < ENGELN_ZERO) ? -FABS(x) : FABS(x))
//namespace basis_h
//{
#define ENGELN_min(a, b)        (((a) < (b)) ? (a) : (b))
/*.IX{min}*/
#define ENGELN_max(a, b)        (((a) > (b)) ? (a) : (b))
/*.IX{max}*/
//}
#define SWAP(typ, a, b) { typ temp; temp = a; a = b; b = temp; }

/* ------------------ Makros von Albert Becker ---------------------- */
#define ABS(X) (((X) >= ENGELN_ZERO) ? (X) : -(X))
#define SIGNY(X,Y) (((Y) < ENGELN_ZERO) ? -ABS(X) : ABS(X))
#define SQR(X) ((X) * (X))
