...
/* CVS $Id: j_cos.cpp,v 1.5 2008/01/23 08:39:36 cxsc Exp $ */
    ...
    { 
      if ((Inf(x)<-q_sint[2])||(Sup(x)>q_sint[2]))
        ...
        { 
          ...
          if (Inf(res)<0)
            {
              Sup(res)=Inf(res)*q_cosm;
              Inf(res)*=q_cosp;
            }
          else
            {
              Sup(res)=Inf(res)*q_cosp;
              Inf(res)*=q_cosm;
            }
        }
    }
           ...
           { 
             if (q1==0)
             {
               if (q2==1)
               {
                 erg1=q_cos1(y1,k1);
                 erg2=q_cos1(y2,k2);
                 if (erg1<erg2) Inf(res)=erg1*q_sinm; else Inf(res)=erg2*q_sinm;
                 Sup(res)=1.0;
               }
               else if (q2==2)
               {
                 Inf(res)=q_cos1(y2,k2)*q_sinp;
                 Sup(res)=1.0;
               }
               else  /* q2==3 */
               {
             ...
             else if (q1==2)
             {
               ...
               else  /* q2==3 */
               {
                 ...
                 erg1=q_cos1(y1,k1);
                 erg2=q_cos1(y2,k2);
                 if (erg1>erg2) Sup(res)=erg1*q_sinm; else Sup(res)=erg2*q_sinm;
               }
             }
