...
/* CVS $Id: l_real.cpp,v 1.20 2008/01/23 08:39:31 cxsc Exp $ */
...
void l_real::_akku_out() throw()
{   
    // The dotakku[0] value is rounded to the activated l_real number
    // in its own precision.
    _clear(1);
    this->elem(1) = rnd(dotakku[0]);
    if (prec>1) 
    {
       int i=2, weiter;
       do {
           dotakku[0] -= this->elem(i-1);
           this->elem(i) = rnd(dotakku[0]);
           i++;
...
void l_real::_akku_out_up() throw()
{   
    // The dotakku[0] value is rounded up to the activated l_real
    // number in its own precision.
    // Blomquist, 20.11.2006;
    bool weiter;
    _clear(1);
    this->elem(1) = (prec==1)? rnd(dotakku[0],RND_UP) : rnd(dotakku[0]);
    if (prec>1) 
    {
       int i=2;
       do {
           dotakku[0] -= this->elem(i-1);
	   weiter = (sign(dotakku[0]) != 0);
	   if (weiter)
	       this->elem(i) = (i==prec)? 
		   rnd(dotakku[0],RND_UP) : rnd(dotakku[0]); 
           i++;
       } while (weiter && (i <= prec));
    }
}

void l_real::_akku_out_down() throw()
{   
    // The dotakku[0] value is rounded up to the activated l_real
    // number in its own precision.
    // Blomquist, 20.11.2006;
    bool weiter;
    _clear(1);
    this->elem(1) = (prec==1)? rnd(dotakku[0],RND_DOWN) : rnd(dotakku[0]);
    if (prec>1) 
    {
       int i=2;
       do {
	   ...
	   if (weiter)
	       this->elem(i) = (i==prec)? 
		   rnd(dotakku[0],RND_DOWN) : rnd(dotakku[0]); 
           i++;
       } while (weiter && (i <= prec));
    }
}
...
{  // Blomquist, 09.12.02; throw() ---> throw(DIV_BY_ZERO)
   ...
   a = rnd(dotakku[0], RND_DOWN);
   b = rnd(dotakku[1], RND_UP);
   if (!b) 
      ...
      for (int i=2; i<=stagprec; i++) 
      {
         ...
         for (int j=1; j<=lr2.prec; j++)
            if (!!lr3.elem(i-1) && !!lr2.elem(j) )
               accumulate(dotakku[0], lr3.elem(i-1), -lr2.elem(j));
         a = rnd(dotakku[0], RND_DOWN);
         lr3.elem(i) = a/b;
      }
   } // no division by zero
