...
/* CVS $Id: l_imath.cpp,v 1.25 2008/01/23 08:39:31 cxsc Exp $ */
...
//               bas, tn, t4,
   ...
   else 
   {
      ...
      {
         m = floor(_double(real(Sup((x/zwopi)))));  // floor rundet zur nÑchsten
...
l_interval atan(const l_interval & x) throw()  // aTan(x)
{
   ...
   else 
   {
	   ...
	   {   
	       lnt2 = Sup(ln(err)); // Erzeugt alten Fehler nicht mehr!! 
         ...
         /*
            lnt = Sup(ln(interval(abs(t)))); // bei Andreas u. Christian "t2"
            lneps = (1.0-digits*stagprec)*lnb;
            do {
               m += 3;
            } while (lneps+ln(2*m+3)-lnt*(2*m+3) <= 0.0);

            bas = real(power(l_real(2.0),1-digits*stagprec));
            tn = abs(real(Sup(t)));
            t3 = abs(real(Sup(power(t,3))));
            do {
               m += 3;
               tn *= t3;
               if (tn == 0.0) 
                  break; // bei Unterlauf erfolgt keine Fehlermeldung!
            } while (bas-tn/m <= 0.0);
         */
	   degree = m;
	   ...
	   relerr[3] = Sup(err);
           ...
           // Argumentreduktion r¸ckg‰ngig machen, mit t mult. 
           // und Fehler einbauen
	   y = zhn*t*p*relerr;
...
//   pihalbe = 2.0*atan(l_interval(1.0));
   ...
   {
      y = pihalbe-atan(x);
      // y = atan(1.0/x);  andere Mîglichkeit der Berechnung
      y = adjust(y);
      y = y & einfachgenau;
   }
...
l_interval sinh(const l_interval & x) throw(ERROR_LINTERVAL_FAK_OVERFLOW)   // Sinh(x)
{
   ...
   else 
   {
      ...
      // Bei Werten Åber 0.5 -- Auswertung Åber e-Funktion
      if (Sup(dy) > 0.5) 
      {
         try {
            t = exp(y);
      ...
      // Auswertung Åber Potenzreihenentwicklung
      else 
      {
         t = y;
         t2 = t*t;

         // AbschÑtzung der Genauigkeit
