...
/* CVS $Id: l_cimath.cpp,v 1.7 2008/01/23 08:39:31 cxsc Exp $ */
...
l_cinterval exp(const l_cinterval& z)
{
...
l_cinterval cos(const l_cinterval& z)
{
    int stagsave = stagprec,
	stagmax = 19;
    l_interval lreal(Re(z)),limg(Im(z));
...
l_cinterval sin(const l_cinterval& z)
{
    int stagsave = stagprec,
	stagmax = 19;
    l_interval lreal(Re(z)),limg(Im(z));
...
l_cinterval cosh(const l_cinterval& z)
{
    int stagsave = stagprec,
	stagmax = 19;
    l_interval lreal(Re(z)),limg(Im(z));
...
l_cinterval sinh(const l_cinterval& z)
{
    int stagsave = stagprec,
	stagmax = 19;
    l_interval lreal(Re(z)),limg(Im(z));
...
l_cinterval sqr(const l_cinterval& z)
...
l_cinterval Tan( const l_cinterval& z ) throw() // -------------- 040827 --
{
  ...
  //  1st: check for poles
  //
    if( ( !disjoint( l_interval(0), imz ) ) and 
...
//  tanh( z ) = transp( i * tan( transp( i * z ) )
//
l_cinterval tanh( const l_cinterval& z )
{
    int stagsave = stagprec,
	stagmax = 19; 
    if (stagprec < stagmax) stagprec++;
...
l_cinterval coth( const l_cinterval& z )
{ // coth( z ) = i * cot( i * z );
    int stagsave = stagprec,
	stagmax = 19; 
    if (stagprec < stagmax) stagprec++;
...
l_interval Atan(const l_interval& y, const l_real& x)
// Calculating an inclusion of atan(y/x) with x<>0.0;
...
l_interval arg( const l_cinterval& z )
{
    l_real
	srez = Sup( Re(z) ),
	irez = Inf( Re(z) ),
    ...
    if( irez < 0.0 and iimz <= 0.0 and simz >= 0.0  )
	    ...
	    //  0 in z and 0 interior point of Re(z)
	{
	    ...
	    resu = ( ( iimz < 0.0 and simz == 0.0 ) ? l_real(0.0) : 
		     Sup( Pi_l_interval() ) );
	    return l_interval( resl, resu );
	}
	else
...
l_interval arg_inclmon( const l_cinterval& z )
{
    if( Inf( Re(z) ) < 0.0 and Inf( Im(z) ) <= 0.0 and Sup( Im(z) ) >= 0.0  )
...
//
{
	...
	else
	{
		...
		if( iimz > 0.0 ) 
		{
		  ...
		  //  max( Re( Root( z ) ) )
		  //
		    if ( irez >= 0.0 or Sup( hyu / irez ) <= tanglel )
		    ...
		    {
			if ( srez < 0.0 and Inf( hyu / srez ) >= tangleu )
		  ...
		  //  min( Im( Root( z ) ) )
		  //
		  if ( srez >= 0.0  or Sup( hyl / srez ) <= tanglel )
...
//
{
    if( Inf( Re(z) ) == 0.0 and Inf( Im(z) ) == 0.0 )
...
// Hilfsfunktion zur Berechnung des Imaginärteils von asin(z),
// z = x + i*y;
// Bezeichnungen:
...

{
    ...
    if (rx>2.0 or ry>2.0) res = acosh( f_aux_asin(x,y) );
    else 
...
l_cinterval asin( const l_cinterval& z ) throw() //------------- 040730 --
{
    ...
    bool bl    = iimz< 0.0 and simz>0.0,
         raxis = iimz==0.0 and simz==0;

  //
  //  1st: check for singularities
  //
    if( (irez<-1 and (bl or (iimz<0  and simz==0))) or 
	(srez >1 and (bl or (iimz==0 and simz>0))) )
...
//	resxu = Sup( asin( hxu / f_aux_asin( hxu, interval( max( - iimz, simz ) ) ) ) );
    ...
    {
      if( ( iimz >= 0.0 and irez >= 0.0  ) or ( simz <= 0.0 and irez <= 0.0 ) )
	...
	//  resxl = Inf( asin( hxl / f_aux_asin( hxl, hyl ) ) );
      ...
      if( ( iimz >= 0.0 and srez >= 0.0  ) or ( simz <= 0.0 and srez <= 0.0 ) )
...
// Asin_arg berechnet für Punktintervalle x,y mit 
// beta := 2*|x| / ( sqrt((x+1)^2+y^2) + sqrt((x-1)^2+y^2) )
// und delta := 1 - |beta|  eine Einschließung von:  
...
//-- asinh ---------------------------------------------------------- 040730 --
//
l_cinterval asinh( const l_cinterval& z )
//
//  asinh( z ) = i * asin( -i * z )
//
{
...
//
{
  ...
  //  1st: check for singularities
  //
    if( ( iimz <= 0.0 and simz >= 0.0 ) and ( irez < 1.0 ) )
...
l_cinterval atan( const l_cinterval& z ) throw() //----- 040912 --
{
  ...
  //  1st: check for singularities
  //
    if( ( irez <= 0.0 and srez >= 0.0 ) and ( iimz <= -1.0 or simz >= 1.0 ) )
...
//  ----------       Improvements for Im(z) = [1,1]  or  Im(z) = [-1,-1]  
    bool sqrImz_1 = (iimz==simz) and (iimz==1.0 or iimz==-1.0); // Test for Im(z) = [1,1] or [-1,-1]
...
//      abs_hxl = sqrt( sqr( abs_hyl ) - 1.0 );
	...
	if( Sup( abs_hxl ) > irez and Inf( abs_hxl ) < srez )
	...
	else if( -Inf( abs_hxl ) > irez and -Sup( abs_hxl ) < srez )
...
//    y_extr = sqrt( 1.0 + sqr( x_extr ) );
    ...
    if( Inf( y_extr ) < simz and Sup( y_extr ) > iimz )
    ...
    if( -Sup( y_extr ) < simz and -Inf( y_extr ) > iimz )
...
l_cinterval acot( const l_cinterval& z ) throw()
{
 ...
 l_interval
  ...
  //  1st: check for singularities
  //
  if( ( irez <= 0.0 and srez >= 0.0 ) and ( iimz <= 1.0 and simz >= -1.0 ) )
...
//	abs_hxl = sqrt( sqr( abs_hyl ) - 1.0 );
	...
	//  intersection in Q I or Q IV: update maximum
      ...
      if( -Inf( abs_hxl ) > irez and -Sup( abs_hxl ) < srez )
	//  extremal curve intersects lower boundary of x+i|y| in quadrant II
...
//-- atanh ---------------------------------------------------------- 040912 --
//
l_cinterval atanh( const l_cinterval& z )
//
//  atanh( z ) = - i * atan( i * z )
//
{
...
//-- acoth ---------------------------------------------------------- 040912 --
//
l_cinterval acoth( const l_cinterval& z )
//
//  acoth( z ) = i * acot( i * z )
//
{
