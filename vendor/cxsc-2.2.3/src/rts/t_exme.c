...
/* CVS $Id: t_exme.c,v 1.11 2008/01/23 08:39:43 cxsc Exp $ */
...
#ifdef ANSI_C
#ifdef LINT_ARGS
int expm1ee (const ExtReal *arg, ExtReal *res)
#else
int expm1ee (arg, res)
const ExtReal   *arg;
      ExtReal   *res;
...
#else  /* NOT ANSI_C */
#ifdef LINT_ARGS
int expm1ee (ExtReal *arg, ExtReal *res)
#else
int expm1ee (arg, res)
ExtReal   *arg;
ExtReal   *res;
...
#endif /* ANSI_C */
{
#ifdef ANSI_C
#if SUN4_CPP_C
   static ExtReal c[8], cm1[8];
   static char cc[8][sizeof(ExtReal)] = ExpKonstanten;
   static char cm1c[8][sizeof(ExtReal)] = Expm1Konstanten;
#else
   /* c[i]=2**(i/8)     */
   static const ExtReal c[8]  =ExpKonstanten;
   /* c[i]=2**(i/8) - 1 */
   static const ExtReal cm1[8]=Expm1Konstanten;
#endif
#else
   static ExtReal c[8], cm1[8];
   static char cc[8][sizeof(ExtReal)] = ExpKonstanten;
...
#ifndef ANSI_C
   {
      ...
      for (i=0; i<8; i++)
         memcpy(&(cm1[i]), cm1c[i], sizeof(cm1[0]));
   }
#else
#if SUN4_CPP_C
   {
