...
/* CVS $Id: d_psub.c,v 1.11 2008/01/23 08:39:40 cxsc Exp $ */

/****************************************************************/
/*                                                              */
/*      Filename        : d_psub.c                              */
/*                                                              */
/*      Entries         : void d_psub(c,a,b)                    */
/*                        d_otpr *c;                            */
/*                        a_real a,b;                           */
/*                                                              */
...
#ifdef LINT_ARGS
local void d_psub(d_otpr *c,a_real a,a_real b)
#else
local void d_psub(c,a,b)
...
a_real b;
                 ...
                 if (expob>EXPO_MAX) {
                    ...
                    if (MANT_INFINITY(mantb)) {
                       if ((rega[A_STATUS] & A_QUIETNAN)==ZERO)
                          {
                          if (NOT(vzc) && (rega[A_STATUS] & A_PINFINITY))
                             {
                          ...
                          else if (vzc && (rega[A_STATUS] & A_MINFINITY))
                             {
                             e_trap(INV_OP+E_IEEE,8,E_TMSG,9,
                                    E_TDTP,c,E_TDBL,&a,E_TDBL,&b);
                             rega[A_STATUS] |= A_QUIETNAN;
                             ...
                             e_trap(NO_ERROR+E_IEEE+E_EMSG+E_ECNT,6,E_TMSG,13,
                                    E_TDBL,&a,E_TDBL,&b);
                             if (NOT(vzc)) rega[A_STATUS] |= A_MINFINITY;
        ...
        if (zeroa || zerob)
           {
           rega[A_STATUS] |= (vzc) ? A_MZERO : A_PZERO;
           E_TPOPP("d_psub")
           return;
           }
        ...
        /* exponent of least signifcant bit of product  */
