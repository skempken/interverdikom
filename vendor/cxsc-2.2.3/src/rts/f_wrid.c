...
/* CVS $Id: f_wrid.c,v 1.11 2008/01/23 08:39:41 cxsc Exp $ */
...
#ifdef LINT_ARGS
local void f_wrid(FILE*   desc,d_otpr r, a_char mode )
#else
local void f_wrid(desc,r,mode )
...
#endif
{
	   static int number_values=(80-4)/(2*sizeof(r[0])+1)+1;
	   char format[10];

        E_TPUSH("f_wrid")


	      a_btyp i,k, start, end;

           strcpy(format,(sizeof(r[0]) ==4)?"%08l":"%016l");

		 if (mode =='x' || mode =='X' ) 
		 {
             strcat(format,(mode=='x')?"x ":"X ");

             start= A_START+1;
#ifdef A_LENGTH_64
		   end  = (a_btyp) A_LENGTH_64-1; 
#else
		   end  = (a_btyp) A_LENGTH-1; 
#endif
		 }
		 else
		 if (mode =='z' || mode =='Z' ) 
		 {
             strcat(format,(mode=='z')?"x ":"X ");
             start = (a_btyp) r[A_BEGIN]; 
		   end   = (a_btyp) r[A_END]; 
		 }
		 else
		 {
		 ...
		 fprintf(desc,"Status Words :");
           for (k=0,i=A_BEGIN; i<=A_START; i++,k++)
		 {
		   if (k%number_values == 0) { fprintf(desc,"\n%03ld|",(long)i);k=1;}
		   fprintf(desc,format,(unsigned long)r[i]);
		 }
