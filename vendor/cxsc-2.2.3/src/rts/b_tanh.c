...
/* CVS $Id: b_tanh.c,v 1.12 2008/01/23 08:39:40 cxsc Exp $ */

/*************************************************************************
 *                                                                       *
 * Descriptive Name : Ltanh              Processor : C                   *
 *                                                                       *
 * Hyperbolic Tangent Function for Multiple Precision Arithmetic         *
 * =============================================================         *
 *                                                                       *
...
/****************************************************************
 * Constants for Computation of tanh(x)                         *
 ****************************************************************/
...
#ifdef LINT_ARGS
int b_tanh(dynamic *xi,dynamic *ri)
#else
int b_tanh(xi,ri)
...
#define  LRoutine    "Ltanh"
{
  ...
  /**************************************************************
   * Check and handle case tanh(x)=sgn(x)                       *
   **************************************************************/
  ...
  /**************************************************************
   * Computation of tanh(xi) using exp(xi)                      *
   **************************************************************/
  ...
  /* The final result is computed by squaring the value for the reduced
     ...
     computation of the reduced argument.               */
      ...
      DIV_(&Lone,&val,&oneovexp); /* tanh(xi) =               */
  ...
  /* Computation of sinh(xi) using internal return.
     ...
     rounding error is returned in variable &errsinh ( == LhE ).     */
      ...
      DIV_(&val,cosh,&val);     /* tanh(xi) = sinh(xi)/cosh(xi)  */
...
#ifdef Debug
   if (Ldebug >= 0) {
      printf("\n  (Ltanh 329) |err| = ");  Lprinti(&err);
      printf("\n  (Ltanh 330) val   = ");  Lprinti(&val);
      }
#endif
...
#if INT_HPREC
   b_farg = xi;
   b_case = LESS_ABS_ARG | LESS_ABS_ONE;
#endif
...
#ifdef Debug
   if (Ldebug >= 0) {
      printf("\n  (Ltanh 363) LhF    = ");  Lprinti(&LhF);
      printf("\n  (Ltanh 364) LhE    = ");  Lprinti(&LhE);
      printf("\n  (Ltanh 365) Result = ");  Lprinti(ri);
      printf("   (+ %d ulp)",ri->r);
      }
#endif
