...
/* CVS $Id: t_iath.c,v 1.11 2008/01/23 08:39:43 cxsc Exp $ */
...
/* StdFctInterval(t_iath,iacothee) */
#ifdef LINT_ARGS
a_intv t_iath(a_intv ai)
#else
a_intv t_iath(ai)
...
#ifdef ANSI_C
#ifdef LINT_ARGS
int iacothee (const IExtReal *arg, IExtReal *res)
#else
int iacothee (arg, res)
const IExtReal   *arg;
      IExtReal   *res;
#endif /* LINT_ARGS */
#else  /* NOT ANSI_C */
#ifdef LINT_ARGS
int iacothee (IExtReal *arg, IExtReal *res)
#else
int iacothee (arg, res)
IExtReal   *arg;
IExtReal   *res;
...
#endif /* ANSI_C */
{
   ...
   ArgCheckI1(IAcoth, arg, res);
   if(SGNE(&(arg->u))!=SGNE(&(arg->l))) /* Argument enthaelt Pol */
      return exc_handle_i1(IAcoth, ExcISing, arg, res);
   arg_check = Off;

   /* --- acoth --- */
   retu = acothee(&(arg->l), &r.u);
   retl = acothee(&(arg->u), &r.l);
