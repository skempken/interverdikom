...
/* CVS $Id: t_isnh.c,v 1.11 2008/01/23 08:39:43 cxsc Exp $ */
...
/* StdFctInterval(t_isnh,isinhee) */
#ifdef LINT_ARGS
a_intv t_isnh(a_intv ai)
#else
a_intv t_isnh(ai)
...
#ifdef ANSI_C
#ifdef LINT_ARGS
int isinhee (const IExtReal *arg, IExtReal *res)
#else
int isinhee (arg, res)
const IExtReal   *arg;
      IExtReal   *res;
...
#else  /* NOT ANSI_C */
#ifdef LINT_ARGS
int isinhee (IExtReal *arg, IExtReal *res)
#else
int isinhee (arg, res)
IExtReal   *arg;
IExtReal   *res;
...
#endif /* ANSI_C */
{
   ...
   ArgCheckI1(ISinh, arg, res);
   arg_check = Off;

   /* --- Sinh --- */
   retu = sinhee(&(arg->u), &r.u);
   retl = sinhee(&(arg->l), &r.l);
   ...
   /* --- Rundungs-Fehler --- */
   iround_rel(&r, &EpsSinh, res);

   /* Ergebnisverbesserung in Spezialfaellen */
#if INT_HPREC
   { int vz;
     vz = SGNE(&(arg->l));
     if (vz==POS&&1==cmpee(&(arg->l), &(res->l)))    /* sinh x >= x */
        copyee(&(arg->l), &(res->l));
     vz = SGNE(&(arg->u));
     if (vz==NEG&&-1==cmpee(&(arg->u), &(res->u)))   /* sinh x <= x */
        copyee(&(arg->u), &(res->u));
   }
#endif /* INT_HPREC */
