...
/* CVS $Id: i_cos.c,v 1.11 2008/01/23 08:39:41 cxsc Exp $ */

/****************************************************************/
/*                                                              */
/*      Filename        : i_cos.c                               */
/*                                                              */
/*      Entries         : a_intv i_cos(a)                       */
/*                        a_intv a;                             */
/*                                                              */
...
extern a_btyp LhI;    /* entier(x/(pi/4)) mod 16 after call cos(x) */

#ifdef LINT_ARGS
local a_intv i_cos( a_intv a )
#else
local a_intv i_cos(a)
a_intv a;
#endif
...
static a_btyp o_2pil[] = B_TYPINI( 0x401921fbL, 0x54442d18L );  /* chopped */
        ...
        local a_real *twopil = (a_real *)&o_2pil[0];
        ...
        if (i_point(a)) {
            ...
            rc = i_invp(Lcos,a.INF,&res.INF,&res.SUP);
...
*/
            rc = i_invp(Lcos,a.INF,&clb.INF,&clb.SUP);
            jlb = (unsigned) ((LhI/2)+1);      /* entier(lb/(pi/2)) mod 4 */

/*          printf("entier part Lhi lb mod 16: %u\n", LhI);
*/
            rc += i_invp(Lcos,a.SUP,&cub.INF,&cub.SUP);
            jub = (unsigned) ((LhI/2)+1);      /* entier(lb/(pi/2)) mod 4 */
