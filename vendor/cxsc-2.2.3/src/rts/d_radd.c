...
/* CVS $Id: d_radd.c,v 1.11 2008/01/23 08:39:40 cxsc Exp $ */

/****************************************************************/
/*                                                              */
/*      Filename        : d_radd.c                              */
/*                                                              */
/*      Entries         : void d_radd(c,a)                      */
/*                        d_otpr *c;                            */
/*                        a_real a;                             */
/*                                                              */
...
/*                        a = real value                        */
/*                                                              */
/*      Description     : add real value to dotprecision        */
/*                                                              */
/****************************************************************/
...
#ifdef LINT_ARGS
local void d_radd(d_otpr *c,a_real a)
#else
local void d_radd(c,a)
...
a_real a;
        ...
        if (b_deko( a, &expoa, manta, &vza ))
           {
           (*c)[A_STATUS] |= (vza) ? A_PZERO : A_MZERO;
        ...
        if ( expoa>EXPO_MAX )
                ...
                if (MANT_INFINITY(manta))
                   ...
                   if (((*c)[A_STATUS] & A_QUIETNAN)==ZERO)
                      ...
                      else if (vza && ((*c)[A_STATUS] & A_PINFINITY))
                         {
        ...
        /* add value to dotprecision with different sign        */
                ...
                /* consider leading zeros in case of denormalized numbers */
        ...
        /* always required because of denormalized numbers */
        d = &((*c)[(*c)[A_BEGIN]]);
        while (*d++==ZERO)
                {
