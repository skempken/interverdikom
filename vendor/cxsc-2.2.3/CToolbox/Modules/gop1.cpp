...
/* CVS $Id: gop1.cpp,v 1.4 2008/01/23 08:39:21 cxsc Exp $ */
                             ...
                             interval&  Minimum )
{
  ...
  if ( !UlpAcc(Start,1) ) {
    ...
    do {                                                    // Start iteration
      U[1] = _interval(Inf(Y),c);                           // Bisect 'Y'
      U[2] = _interval(c,Sup(Y));                           //----------------
      for (i = 1; i <= 2; i++) {
      ...
      while ( (WorkList != EmptyList) && !Bisect ) {        // the work list
        ...
        Minimum = _interval(GetFyi(PairY),fmax);
        if ( (RelDiam(Minimum) <= Epsilon) || (RelDiam(Y) <= Epsilon) ) {
          ...
          if (ListLength > MaxOptiNo) {// the number of optimizers.
            FreeAll(WorkList);         //-------------------------------------
            Minimum = _interval(GetFyi(Head(ResultList)),fmax);
            return;
          }
  ...
  Minimum = _interval(GetFyi(Head(ResultList)),fmax);// of the global minimum
} // GlobalOptimize                                  //-----------------------
